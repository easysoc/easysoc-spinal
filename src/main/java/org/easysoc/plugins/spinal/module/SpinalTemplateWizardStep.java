...
public class SpinalTemplateWizardStep extends ModuleWizardStep implements Disposable {
...
//  private final NewProjectWizard myWizard;
  ...
  public SpinalTemplateWizardStep(WizardContext context, SettingsStep settingsStep) throws NoSuchFieldException, IllegalAccessException {
    myContext = context;
//    myWizard = (NewProjectWizard)context.getWizard();
  ...
  private void setTemplatesList() {
    for (TemplatesGroup group : myTemplatesMap.keySet()) {
      if (group.getName().equals("SpinalHDL")) {
